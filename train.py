...
if __name__ == "__main__":
    ...
    batch_size = 128
    epochs = 500
    subset_fraction = 0.5  # Use 80% of the data in each epoch
    ...
    # Create dynamic sampler for shuffling data every epoch
    def create_dynamic_sampler(dataset, train_indices):
        # random.shuffle(train_indices)  # Shuffle indices
    ...
    # Initialize train_loader with a dynamic sampler
    train_indices = list(train_idx)  # Ensure train_idx is mutable

    # Training loop
    best_val_loss = float('inf')
    for epoch in range(epochs):

        # Dynamically shuffle data and recreate sampler
        train_sampler = create_dynamic_sampler(full_dataset, train_indices)
        ...
        for images, labels in tqdm(train_loader, desc=f"Training Epoch {epoch + 1}"):
            ...
            optimizer.step()
            scheduler.step()
            # Log learning rate for this batch
